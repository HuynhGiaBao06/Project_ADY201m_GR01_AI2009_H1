...
* **Thực thi:** 
  ...
  * Chỉ sử dụng SQL để viết các câu lệnh truy vấn (`SELECT`, `JOIN`, `WHERE`) nhằm trích xuất tập dữ liệu theo nhóm (ví dụ: Churn theo Loại hợp đồng, Phương thức thanh toán) và đẩy vào môi trường Python. Không thực hiện xử lý logic hay tính toán phức tạp trên SQL.

### 5.2. Tiền xử lý, Trực quan hóa & Phân tích bằng Python (Data Processing & Visualization)
Toàn bộ logic làm sạch dữ liệu, EDA và trực quan hóa đều được thực hiện trên Python.
* **Tiền xử lý:** Dùng `pandas` để làm sạch các giá trị bị thiếu (*missing values*), xử lý dữ liệu ngoại lai (*outliers*), mã hóa các biến phân loại (*Categorical encoding*) và chuẩn hóa các biến số (*Feature scaling*). Thực hiện Feature Engineering tạo các biến mới như kế hoạch.
* **Trực quan hóa:** Sử dụng `seaborn` và `matplotlib`:
  * **Bar Chart:** Trực quan hóa Churn Rate theo loại hợp đồng, dịch vụ.
  * **Boxplot:** Phân tích cước phí (Monthly Charges) giữa nhóm churn và non-churn.
  * **Heatmap:** Kiểm tra tương quan giữa các đặc trưng khách hàng.
